...
There are several reasons for Python's popularity. Some of them are:
(a) Free:
...
Objects interact with one another by sending messages to each other by calling each other's interface methods.
